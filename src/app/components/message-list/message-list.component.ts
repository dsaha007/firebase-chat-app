  ...
  template: `
    <div class="message-list" #messageContainer>
      <div class="chat-info" *ngIf="selectedUser">
        <div class="user-avatar" [style.background-color]="selectedUser.avatar || '#ccc'">
      ...
      <ng-container *ngIf="messages | async as messageList">
        ...
        <div class="no-messages" *ngIf="messageList.length === 0 && !loading">
          <p *ngIf="!selectedUser">No messages in the public chat yet. Be the first to send a message!</p>
          <p *ngIf="selectedUser">No private messages with {{ selectedUser.name }} yet. Start the conversation!</p>
        </div>
      </ng-container>
    </div>
  `,
